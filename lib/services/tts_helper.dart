...
Future<Style> loadVoiceStyle(List<String> paths) async {
  ...
  if (paths[0].startsWith('assets/')) {
    firstContent = await rootBundle.loadString(paths[0]);
  } else {
    firstContent = await File(paths[0]).readAsString();
  }
  final firstJson = jsonDecode(firstContent);
  ...
  for (var i = 0; i < bsz; i++) {
    ...
    if (paths[i].startsWith('assets/')) {
      content = await rootBundle.loadString(paths[i]);
    } else {
      content = await File(paths[i]).readAsString();
    }
    final json = jsonDecode(content);
...
Future<Map<String, OrtSession>> _loadOnnxAll(String dir) async {
  ...
  final sessions = await Future.wait(models.map((name) async {
    ...
    if (dir.startsWith('assets/')) {
      // For bundled assets, use createSessionFromAsset
      path = '$dir/$name.onnx';
      return ort.createSessionFromAsset(path);
    } else {
      // For file system paths, use createSession directly
      path = '$dir/$name.onnx';
      print('Loading ONNX model from file: $path');
